...
\date{Version 0.8, 2015-April-25}
\supervisor{Dr. Radu Calinescu}
\BEng
\wordcount{19044}


\abstract{ As the Open Data movement continues to gain momentum, more and more web applications are being developed to exploit this data; creating useful tools for governments, businesses and the general public. Road safety data published by the UK Department for Transport is used by several web applications, all of which are very similar in purpose. These applications lack advanced search functionality, so it is not possible to analyse a specific route. The application developed for this project searches for collisions that have occurred on user-specified road journeys. The results are used to highlight collision hotspots on the route and calculate useful statistics. These results can be used as a basis for comparing the safety of different roads. The final version of the application is publicly available and can be accessed at \textit{routerisks.co.uk}. It has also been submitted to feature in the application catalogue on \textit{data.gov.uk}. }
...
User participation was required for evaluations of the application. This was a simple procedure and thus did not raise any serious ethical concerns. Participants were briefed about the evaluation before signing a consent form confirming their understanding. All data obtained was treated anonymously.
...
\begin{table}[tbp]
  ...
  \begin{tabular}{ | p{2.2cm} |  p{3.0cm} | p{2.2cm} |  p{2.0cm} | p{3.5cm} | }
    ...
    Crash Map & UK Road Safety 05-13 & Web & Google Maps & Severity, Casualty Types, Year\\ \hline
	Collision Map & UK Road Safety 05-13 & Web & Google Maps & Highway Authority, Year, Severity, Casualty Age, Vehicle Type\\ \hline
	Motorcycle Accidents & UK Road Safety 2011 (Motorcycle Only) & Web & N/A & N/A\\ \hline
...
\begin{table}[tbp]
  ...
  \begin{tabular}{ | l | l | l | l | }
      ...
      Popularity & 1 & 3 & 2 \\ \hline
      Learnability & 2 & 3 & 1\\ \hline
	  Security & 2 & 1 & 1 \\ \hline
...
The application developed has been made publicly available at \textit{routerisks.co.uk}. In order to advertise the application, it was submitted to \textit{data.gov.uk}, which includes a library of 350+ applications that use Open Data. It is envisaged that people interested in Open Data will find the application here and use it to analyse routes of interest to them. Councils may also find the application, and use it to analyse which roads are the most dangerous within their county. It is hoped that the application can be used to assist decisions made regarding which roads need safety improvements. 

The technologies used for the application were all open-source, with the exclusion of the Google Maps API. The application is deployed using a LAMP environment, meaning it uses Linux (Ubuntu), Apache, MySQL and PHP; all of which are open-source. The Google Maps API is free to use for publicly available sites (providing the usage limits are not exceeded), so the only costs associated with the application are for web hosting. 
